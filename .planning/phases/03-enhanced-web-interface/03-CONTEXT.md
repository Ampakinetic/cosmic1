...
- **D-41:** All six alert conditions evaluated BASE-SIDE from data the base already has — altitude/GPS/temperature from the received 0x14 telemetry beacon, RSSI/SNR from the base's own E32 receiver, GPS-lost from fix age, ascent rate from altitude deltas, landing from altitude trend. Zero extra balloon airtime. **Researcher must confirm battery voltage (ALRT-02) rides the telemetry beacon — if not, adding it to the beacon payload is the one permitted balloon-side change in this phase.**
  - **Amendment (2026-08-20, plan revision — pending user ratification):** Research Finding 2 (03-RESEARCH.md; author-sourced from E32 900T30D documentation, cross-checked twice) shows the E32 exposes neither RSSI nor SNR over its UART interface, so the clause "RSSI/SNR from the base's own E32 receiver" is not implementable as written. The ALRT-06 signal-quality input is amended to a derived beacon-loss metric — percent of telemetry beacons missed over a sliding 60 s window, computed from the beacon's existing BE16 sequence-number gaps (implemented in 03-03 Task 1; the UI displays this metric only and never an RSSI/SNR value). ALRT-06 remains satisfied, and the decision's core — all six alerts evaluated base-side from data the base already has, zero extra balloon airtime — is unchanged. The original decision text above is preserved verbatim; this amendment is surfaced to the user for ratification in the plan-check summary.
