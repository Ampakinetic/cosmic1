...
updated: 2026-08-18T11:14:05Z
---

## Current Test

number: 1
name: End-to-end command round trip over real radios
expected: |
  With both units flashed and powered (balloon + base station, E32 radios connected),
  trigger a camera capture from the base station web UI. The command is acknowledged
  (ACK) within the 2 s CAPTURE_NOW window, the Command Queue row for the command
  updates to its terminal state, and the ACK/counter statistics advance.
