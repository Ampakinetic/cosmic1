...
- gap_id: G-01-7
  ...
  artifacts:
    - path: "src/image_tx_manager.cpp"
      issue: "new-capture supersede/eviction rule (the 'window request for image N supersedes older entry' path) — the pending-heal eviction is the primary lever. 01-12: evict-guard landed (1064480); session 4 shows zero mid-service evictions"
    - path: "src/image_rx_manager.cpp"
      issue: "heal gating behind the active full pull (gates 1/2, :190-216) + immediate window re-request cadence — the serialization / RX-settle levers. 01-12: hold + settle landed (1064480), hold verified 3x on the bench; RESIDUAL: the D-24 pass accounting consumes deferred (BUSY-NACK'd) window requests — defer-aware pass charging is the named next lever"
  missing:
    - "Defer-aware D-24 pass accounting: a window request NACK'd by the balloon's one-at-a-time BUSY deferral must not consume a heal pass (session 4: image 7 thumb 35/36 exhausted its 3 passes on deferred+retried requests)"
    - "Full-manifest re-announce (bounded) so a single lost FULL manifest cannot silently drop a full (session 4: image 8)"
    - "G-01-9 thumb-sizing fix — full-sized thumbs multiply push/heal exposure 4-18x and are the compounding cause of the series-A INCOMPLETEs"
    - "Bench re-verification: back-to-back unspaced captures complete without INCOMPLETE verdicts (series A re-run on remediated firmware)"
...
- gap_id: G-01-9
  ...
  missing:
    - "Fix defect A: payload-size/JPEG-dimension verification in the thumbnail guard (or drain the fb queue after the QQVGA downshift and capture the next frame) — discriminator for the mechanism first: log fb->len alongside the dimension check at bench"
    - "Discriminate defect B: add kind to the balloon 'window chunk(image N, ...)' log line and/or a per-window served-bytes CRC, then re-run the series-A/SVGA captures"
    - "Fix defect C: bounded FULL-manifest re-announce so a single lost manifest cannot silently drop a full"
    - "Fix the CommandSender retry overrun (13x 'attempt 4/3' in base4.log) and re-verify the D-05 3-retry bound"
    - "Bench re-verification: series A + CIF/SVGA captures with correctly-sized thumbs completing both kinds COMPLETE"
  opened_at: 2026-08-24
