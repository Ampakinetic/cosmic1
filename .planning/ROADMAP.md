...
**Plans:** 35/36 plans executed through round #15's 01-35 (round #15 planned 2026-09-10 as 01-35..01-36 per the round-#15 routing: the G-01-7 capacity-aware admission lever — 01-35 EXECUTED code-level, builds 2/2 + harness exit 0, zero bench claims, G-01-7 STAYS OPEN — plus the §38.10 [SCHEDSNAP] interrogation shared bench session as 01-36, the round's remaining plan; sessions 12-35 of the D1 campaign ran through the debug workflow per the debug doc §12-§38, WINDOWS/UAT ledgers last touched at 01-35). Round #14 executed as 01-32..01-34 after round #13 (01-31 bench session #10 FAILED at the D1 FOURTH expression — an IDLE0 stack-canary panic with a full dump at chunk 12/14 of image 43's re-armed FULL window, balloon4.log:1428-:1429; the round-#13 instruments ANSWERED: [IDLE0] zero frozen lines = starvation DISCONFIRMED, [TWDT]=ESP_ERR_NOT_FOUND puts §7's WDT premise under re-examination, [I2C] write-path coverage gap; series A finally ran with ALL round-#10 discriminators engaged for the first time — 6/8 verdicts, G-01-7 residual NAMED: depth-3 queue cycling evicts the base-activated prior full; NOTHING FLIPPED, WINDOWS 2/17/19). Gap-closure round #14 planned 2026-08-28 as 01-32..01-34, per the round-#14 routing ("[STACK] watermark instrument (debug doc §10); ledgers updated, STATE carries the round-#4 brief"): 01-32 the D1 debug round #4 instrument package ([STACK] IDLE0-watermark + [TWDT] handle fix + [IDLE0] printf fix + [I2C] write-path coverage + the A/B hook guard; NO D1 lever until [STACK] answers, §10.5 item 5) -> 01-33 the G-01-7 admission/oldest-eviction lever round (routed "AFTER D1 round #4"; the queue-depth + supersede receipt-evidenced protection lever against the named residual) -> 01-34 bench re-verification session #11 (the [STACK]/[TWDT]/[I2C] instrument answers + the D1 three-expression ladder + series A on the lever + the riding clauses SC-3/WR-03/QVGA-restore/dashboard LOOK). Then the security gate
...
- [x] 01-32-PLAN.md — D1 debug round #4 (BLOCKER, G-01-10/WINDOWS 15, fourth expression): the §10.5 instrument package — [STACK] IDLE0-watermark (uxTaskGetStackHighWaterMark from the existing idle hook, tick-throttled, new-low latch — the [MEM] pattern applied to IDLE0; the one-session discriminator for the labeled stack-capacity hypothesis), [TWDT] handle fix (xTaskGetIdleTaskHandleForCPU(0)) + the deployed WDT-config re-derivation, [IDLE0] registered printf fix (err==ESP_OK), [I2C] write-path coverage, the G01_D1_IDLE_HOOK_DISABLED A/B guard; debug doc §11 implementation record + the 01-34 discriminator table; NO D1 lever (§10.5 item 5's guard); builds + harness, no bench claims (CTRL-01, CTRL-06, PRI-02)
- [x] 01-33-PLAN.md — G-01-7 burst full-delivery lever round (WINDOWS 3, the NAMED depth-3 queue-cycling residual, routed "AFTER D1 round #4"): the lever-selection audit (supersede-path source reading, depth-raise PSRAM arithmetic, ranked menu) into 01-G01-7-LEVER.md, the selection checkpoint, then IMG_TX_QUEUE_DEPTH 3->5 + supersede-path receipt-evidenced protection through evictionClassOf with engagement discriminator lines; composes with the 01-21/01-22 levers; builds + harness + PRI-01 order-gates, no bench claims (CTRL-01, CTRL-03, CTRL-06, PRI-02)
- [x] 01-34-PLAN.md — Bench re-verification session #11: the [STACK]/[TWDT]/[I2C] instrument answers ([STACK] confirms/refutes the stack-capacity hypothesis; [TWDT] resolves the two-reading question), the D1 three-expression survival ladder, series A on the admission lever (3/3 ACKs, 6/6 COMPLETE, lever discriminators), WR-03 cadence discriminator + SC-3 visible-effect pairs + QVGA restore + dashboard LOOK (tenth round riding), ledger flips on evidence; STATE routes the next action (the watermark-informed D1 fix round, or the security gate) (CTRL-01, CTRL-02, CTRL-03, CTRL-04, CTRL-06, PRI-02)

**Wave 24** *(gap-closure round #15, 2026-09-10; blocked on the executed rounds — 01-35 is the code round, 01-36 the shared bench trailing wave; D1 sessions 12-35 ran between rounds via the debug workflow, debug doc §12-§38)*

- [x] 01-35-PLAN.md — G-01-7 capacity-aware supersede admission lever (WINDOWS 3, the session-11 over-rejection mode): free-slot capacity gate at the supersede call site + class-0 fully-receipted safe-reclaim class in the SHARED evictionClassOf (WINDOW_COMPLETE fullAcked evidence, no fork) + named discriminator line; WR-02 companion verified already-fixed upstream; builds + harness + PRI-01/protected-surface gates, no bench claims (CTRL-01, CTRL-03, CTRL-06, PRI-02)
- [ ] 01-36-PLAN.md — Bench re-verification session #36 (shared): the §38.10 [SCHEDSNAP] scheduler-state interrogation (card-out, >=6 captures, resolution pinned, P1-P5 pre-registered dispositions, signature-first classification) as primary contract, then G-01-7 series A on the 01-35 lever (3/3 ACKs, 6/6 COMPLETE, lever lines), SC-3 pairs (eleventh round riding), WR-03 cadence discriminator, dashboard LOOK on the current tabbed dashboard; QVGA-restore + reboot-boot-resolution DEFERRED to the post-D1 card-in confirmation run (pinned-resolution controlled variable); §39 + ledger flips on evidence (CTRL-01, CTRL-02, CTRL-03, CTRL-04, CTRL-06, PRI-02)
